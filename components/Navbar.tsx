  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-black/60 border-b border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link
          href="/"
          className="text-2xl md:text-3xl font-black tracking-wide z-10 relative"
        >
          Friend&apos;s <span className="text-red-500">Liquor</span> Store
        </Link>

        <nav className="hidden lg:flex gap-6 xl:gap-8 text-white/70 h-full items-center">
          <Link href="/" className="hover:text-red-500 transition h-full flex items-center">
            Home
          </Link>

          {/* Shop mega menu — categories + premium drinks only */}
          <div className="group h-full flex items-center">
            <Link href="/shop" className="hover:text-red-500 transition py-4">
              Shop
            </Link>
          ...
          <div className="group h-full flex items-center">
            <Link
              href="/premium-drinks"
              className="text-yellow-400 font-bold tracking-widest uppercase text-sm hover:text-white transition py-4"
            >
              Premium Drinks
            </Link>
        ...
        <div className="flex gap-3 md:gap-4 items-center z-10 relative">
          {!loading && user ? (
            <div className="flex items-center gap-3">
              <Link
                href="/profile"
                className="text-white/80 hidden md:inline text-sm hover:text-white"
              >
          ...
          <Link
            href="/cart"
            className="px-4 py-2 rounded-full border border-white/20 bg-white/5 hover:bg-red-500 transition-all flex items-center gap-2"
          >
          ...
          <Link
            href="/checkout"
            className="hidden sm:inline-block px-4 py-2 rounded-full bg-red-500 hover:bg-red-600 transition-all shadow-[0_0_30px_rgba(255,0,0,0.5)] text-sm font-bold"
          >
            Checkout
          </Link>
        </div>
